# 안녕하세요, 김도현입니다

**서울대학교 심리학과** 학부 3학년 (2022-13170) | **뇌-마음-행동 연계전공** 신청 중

---

## 🔬 연구 관심사

**정신병리(주요우울장애, 조현병 등)의 신경생물학적 기제를 뇌영상(fMRI)과 기계학습을 결합하여 개인차 수준에서 이해하고 예측하는 것**에 관심이 있습니다.

특히, 생태순간평가(EMA)로 포착한 일상의 정서 변동성을 뇌 회로 모델과 연동하여, 개인별 병리적 임계점을 규명하고 맞춤형 개입을 설계하는 방향을 추구하고 있습니다.

---

## 🧩 주요 프로젝트

### 🔗 프로젝트 흐름: 이론 모델링 → 자기 추적 도구 개발 → 뇌-행동 연계 분석

```
[조현병 PE-회로 모델] ──>  [EMA 앱 개발]  ──>  [MDD 삼각구조 대시보드]
  (이론/시뮬레이션)      (데이터 생성)          (통합 분석)
```

| 프로젝트 | 기술 | 핵심 기여 | 기간 |
|----------|------|-----------|------|
| **[🧠 조현병 예측오차-신경회로 모델링](https://github.com/kji3216-hash/bmb-schizophrenia-circuit)** | Python, Streamlit, Plotly | 14개 논문 기반 다차원 원인 시뮬레이터 + 3D 뇌 회로 가시화 + 비선형 상전이 모델 | 2026.03 |
| **[🌱 MoodGarden](https://github.com/kji3216-hash/moodgarden)** | HTML/CSS/JS, PWA, Electron | EMA 기반 우울 증상 자기 추적 앱 — 정원 게이미피케이션 + 상관분석 Insights | 2026.05 – 현재 |
| **[📊 MDD 삼각구조 대시보드](https://github.com/kji3216-hash/mdd-moodgarden-ema-dashboard)** | Python, Streamlit, Pandas | MoodGarden EMA 데이터를 MDD 유전-뇌-행동 삼각구조와 연동 분석 | 2026.05 - 현재 |

---

## 💻 기술 스택

**Languages**: R (기초)    
**Tools**: Agentic AI, github, R studio

---

## 📚 관련 수강 과목

심리통계학 · 이상심리학 · 뇌 구조와 기능 · 실험심리학 · 중독의 심리학 · 뇌-마음-행동 교과목

---

## 📫 연락처

- 📧 **Email**: kji3216@snu.ac.kr
- 🏫 **학번**: 2022-13170
- 🌐 **GitHub**: [github.com/kji3216-hash](https://github.com/kji3216-hash)

---
